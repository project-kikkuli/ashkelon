...
fn claude_wires_the_mcp_channel_by_default() {
    ...
    assert!(plan
        .args
        .contains(&"--dangerously-load-development-channels".to_string()));
    assert!(plan.args.contains(&"server:ashkelon".to_string()));

    assert_eq!(
        plan.wake.control.as_deref(),
        Some(dir.join("launch").join("chan1.sock").to_string_lossy().as_ref())
    );
...
fn claude_no_channel_flag_skips_the_mcp_wiring() {
    ...
    let plan = launch::plan(
        ...
        BASE,
        "chan2",
        &["--no-channel".to_string(), "--continue".to_string()],
        &options(&dir),
    )
    .unwrap();
...
fn codex_openai_api_flag_switches_base_and_strips_itself() {
    ...
    let plan = launch::plan(
        ...
        BASE,
        LAUNCH,
        &["--openai-api".to_string(), "extra-arg".to_string()],
        &options(&dir),
    )
    .unwrap();
    let joined = plan.args.join(" ");
...
fn opencode_writes_temp_config_and_defers_control_to_companion() {
    ...
    let caps = companion
        .ready_pattern
        .captures("opencode server listening on http://127.0.0.1:54321")
        .unwrap();
...
fn opencode_companion_url_resolves_into_args_and_wake_control() {
    ...
    assert_eq!(
        plan.args,
        vec!["attach".to_string(), "http://127.0.0.1:54321".to_string()]
    );
